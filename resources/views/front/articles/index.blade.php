...
@section('content')
    <header class=" relative mt-14 lg:mt-0">
        ...
        <div class="absolute flex  pb-1 pt-5 lg:py-3 flex-col justify-end items-center inset-0 text-white text-center">
            <h1 class=" lg:text-3xl font-semibold">مقالات</h1>
            <div class="w-1 h-6 my-1 lg:my-4 bg-secondary"></div>
            <h2 class="mx-3 lg:w-1/3 text-lg text-sm lg:text-2xl border-2 border-white py-3 px-4">آخرین مقالات ما</h2>
    ...
    <section class="container space-y-3 lg:space-y-6 py-3 lg:py-16">
        ...
        <div>
            <h3 class="font-semibold text-2xl lg:text-4xl text-primary border-r-4 lg:border-r-0 border-primary lg:pr-0 pr-3">مقالات ( {{ request()->category_id !== 'all' && request()->has('category_id') ? $category_get->title : 'همه' }} )</h3>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-1 lg:gap-3 col-span-4">
                @forelse($articles as $article)
                    <div class="border transition-shadow group duration-300   overflow-hidden flex flex-col pb-4">
                        <div class="aspect-[3/2] overflow-hidden"><img src="{{ asset($article->thumbnail)  }}" alt="{{ $article->title }}" class="aspect-[3/2] object-cover transition-transform duration-500 transform group-hover:scale-105">
                        </div>
                        <div class="mt-4 lg:mt-8 space-y-4 flex flex-col lg:px-6 px-3 lg:pb-4  ">
                            ...
                            <div class="flex items-center justify-between mt-4 text-gray-500 text-sm">
                                <span class="font-light">{{ \Morilog\Jalali\Jalalian::forge($article->created_at)->format('Y/m/d') }}</span>

                                <a href="{{ route('front.single.articles',['slug'=>$article->slug])  }}" class="lg:py-2 py-1 px-3 lg:px-6 mt-1 border border-primary text-primary font-medium rounded-md transition-all duration-300 hover:bg-primary hover:text-white focus:ring-2 focus:ring-primary">
                                    ادامه مطلب
                                </a>
                            </div>
                        </div>
